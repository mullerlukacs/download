...
const platform = platforms.find((item) => item.slug === "tiktok");
export const metadata = createMetadata({ title: platform?.name ?? "Platform", description: platform?.description ?? "MediaSave Pro platform tools.", path: "/tiktok" });

export default function PlatformPage() {
  if (!platform) notFound();
  return <main className="container py-16">
    <JsonLd data={{ "@context": "https://schema.org", "@type": "BreadcrumbList", itemListElement: [{ "@type": "ListItem", position: 1, name: "Home", item: "/" }, { "@type": "ListItem", position: 2, name: platform.name, item: "/tiktok" }] }} />
